...
describe("AMM Tests", () => {
  ...
  it("should not allow swaps with insufficient input amount", () => {
    createPool();
    addLiquidity(alice, 1000000, 500000);

    const { result } = swap(alice, 0, true);

    expect(result).toBeErr(Cl.uint(205)); // ERR_INSUFFICIENT_INPUT_AMOUNT
  });

  it("should not allow removing more liquidity than owned", () => {
    createPool();
    addLiquidity(alice, 1000000, 500000);

    const { result } = removeLiquidity(alice, 1000000); // More than owned

    expect(result).toBeErr(Cl.uint(203)); // ERR_INSUFFICIENT_LIQUIDITY_OWNED
  });

  it("should not allow creating pools with incorrect token ordering", () => {
    // Try to create pool with tokens in wrong order (token1, token0 instead of token0, token1)
    const { result } = simnet.callPublicFn(
      "amm",
      "create-pool",
      [mockTokenTwo, mockTokenOne, Cl.uint(500)], // Wrong order
      alice
    );

    expect(result).toBeErr(Cl.uint(201)); // ERR_INCORRECT_TOKEN_ORDERING
  });
  ...
  it("should allow multiple users to add liquidity", () => {
    ...
    const { result } = addLiquidity(bob, 500000, 250000);
    expect(result).toBeOk(Cl.bool(true));
  });

  it("should calculate pool data correctly", () => {
    createPool();
    addLiquidity(alice, 1000000, 500000);
